  ...
  return (
    <>
      <nav className="sticky top-0 z-50 bg-black/80 backdrop-blur-md border-b border-white/5 px-4 py-3 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className="w-9 h-9 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center hover:border-zinc-700 transition-colors cursor-pointer"
          >
      ...
      <UserProfileModal 
        ...
        onSave={(data) => {
          // Optional: You could trigger a reload or state update here if needed
          if (data.campus && data.campus !== campus && window.location.reload) {
             window.location.reload(); // Simple way to refresh context if campus changes
          }
        }}
      />
    </>
